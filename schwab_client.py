...
class SchwabWebSocketClient:
    ...
    def __init__(self, debug: bool = False, symbols_filepath: str = 'symbols.txt', 
        ...
        self.option_symbols = self.load_symbols_from_file(option_symbols_filepath)
        ...
        self.market_open_time = datetime.strptime('09:30:00', '%H:%M:%S').time()
    ...
    def load_symbols_from_file(self, filepath: str) -> List[str]:
        ...
        try:
            if not os.path.exists(filepath):
                print(f"⚠️ Symbols file {filepath} not found. Using empty list.")
                return []
            ...
            with open(filepath, 'r') as f:
                ...
                print(f"✅ Loaded {len(symbols)} symbols from {filepath}: {', '.join(symbols)}")
                return symbols
    ...
    def get_access_token(self) -> str:
        ...
        try:
            if not os.path.exists(self.access_token_filepath):
                raise Exception(f"Access token file not found: {self.access_token_filepath}")
            
            with open(self.access_token_filepath, 'r') as f:
                token = f.read().strip()
            
            if not token:
                raise Exception(f"Access token file is empty: {self.access_token_filepath}")
            
            # Return token as-is (no character removal)
            return token
            
        except Exception as e:
            print(f"❌ Error reading access token from {self.access_token_filepath}: {e}")
            raise
    ...
    def wait_for_market_open(self):
        ...
        if current_date_et.weekday() >= 5:  # Saturday = 5, Sunday = 6
            print(f"📅 Weekend detected ({current_date_et.strftime('%A')}) in ET timezone, market is closed")
            return False
        ...
        if current_time_et < self.market_open_time:
            ...
            if wait_seconds > 0:
                ...
                print(f"⏰ Before market hours (ET time). Current ET time: {now_et.strftime('%H:%M:%S %Z')}")
                ...
                while time.time() - start_time < wait_seconds:
                    ...
                    remaining = (market_open_dt - remaining_now_et).total_seconds()
                    if remaining <= 0:
                        break
                    ...
                    print(f"\r⏳ Waiting: {remaining_hours:02d}:{remaining_minutes:02d}:{remaining_secs:02d} remaining (ET time)...", end='', flush=True)
                    time.sleep(1)
                
                print(f"\n🎉 Market is now open (ET time)! Starting streaming...")
            else:
                print(f"✅ Market is already open (current ET time: {now_et.strftime('%H:%M:%S %Z')})")
        else:
            print(f"✅ Market is already open (current ET time: {now_et.strftime('%H:%M:%S %Z')})")
    ...
    def parse_option_data(self, content_item: dict) -> Dict:
        ...
        try:
            ...
            previous_values = self.previous_option_values.get(symbol, {
                ...
                'quote_time': 0,          # 38: Quote Time (milliseconds since Epoch)
                'trade_time': 0,          # 39: Trade Time (milliseconds since Epoch)
            ...
            for field_key, field_name in field_mapping.items():
                if field_key in content_item:
                    ...
                    if field_name in ['bid_price', 'ask_price', 'last_price', 'high_price', 'low_price', 
                        ...
                        try:
                            option_data[field_name] = float(value) if value else 0
                        ...
                        try:
                            option_data[field_name] = int(value) if value else 0
                        except (ValueError, TypeError):
                            option_data[field_name] = 0
                    elif field_name == 'is_penny_pilot':
                        option_data[field_name] = bool(value) if value else False
                    else:
        ...
        try:
            ...
            if 'time' in data and isinstance(data['time'], int) and data['time'] > 0:
                try:
                    ...
                    timestamp_dt = pd.Timestamp(data['time'], unit='ms', tz='US/Eastern')
                    # Add ET time column (keep original time field unchanged)
                    data['time_et'] = timestamp_dt.strftime('%Y-%m-%d %H:%M:%S %Z')  # type: ignore
    ...
    def subscribe_options_data(self, symbols: List[str]):
        ...
        try:
            ...
            print(f"✅ Subscribed to LEVELONE_OPTIONS data for: {', '.join(symbols)}")
                ...
                if "response" in data:
                    ...
                    if response_data.get("command") == "LOGIN":
                        ...
                        if code == 0:
                            ...
                            if self.symbols:
                                ...
                                print("⚠️ No equity symbols loaded, skipping chart data subscription")
                            
                            if self.option_symbols:
                                self.subscribe_options_data(self.option_symbols)
                            else:
                                print("⚠️ No option symbols loaded, skipping options data subscription")
                        else:
                    ...
                    for data_item in data["data"]:
                        ...
                        if service == "CHART_EQUITY":
                            if self.debug:
                                print(f"📈 Received CHART_EQUITY data: {len(content)} items")
                            ...
                            for candle_data in content:
                                ...
                                if parsed_candle and 'key' in parsed_candle:
                                    ...
                                    if symbol in self.symbols:
                                        # Save to CSV
                                        self.save_chart_data_to_csv(symbol, parsed_candle)
                                    elif self.debug:
                                        print(f"⚠️ Symbol {symbol} not in subscription list, skipping save")
                        
                        elif service == "LEVELONE_OPTIONS":
                            if self.debug:
                                print(f"📊 Received LEVELONE_OPTIONS data: {len(content)} items")
                            ...
                            for option_data in content:
                                ...
                                parsed_option = self.parse_option_data(option_data)
                                if parsed_option:
                                    # Get symbol from parsed data or raw data
                                    symbol = parsed_option.get('symbol', '') or option_data.get('key', '')
                                    if not symbol:
                                        continue
                                    
                                    # Normalize symbols for comparison (remove spaces)
                                    normalized_symbol = symbol.replace(' ', '')
                                    normalized_option_symbols = [s.replace(' ', '') for s in self.option_symbols]
                                    ...
                                    if normalized_symbol in normalized_option_symbols or any(norm_sym in normalized_symbol for norm_sym in normalized_option_symbols):
                                        ...
                                        if parsed_option.get('last_price', 0) != 0 or parsed_option.get('bid_price', 0) != 0:
                                            self.save_options_data_to_csv(symbol, parsed_option)
                                    elif self.debug:
                                        print(f"⚠️ Symbol {symbol} not in option subscription list, skipping save")
